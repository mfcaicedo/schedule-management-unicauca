...
export class OfertaAcademicaService {
  ...
  downloadTemplateService(programCode: String): Observable<any> {
    ...
    return this.http
      .get<any>(this.endPoint + `/downloadTemplate/${programCode}`, {
        responseType: 'json',
      })
      ...
      .pipe(
        catchError((e) => {
          console.log(
            'Error obteniendo la plantilla de la oferta académica',
            e.error.mensaje,
            'error'
          );
          return throwError(e);
  ...
  findAllFiles(page: number, pageSize: number): Observable<any> {
    return this.http
      .get<any>(
        this.endPoint +
          `?page=${page - 1}&size=${pageSize}&sort=dateRegisterFile&order=ASC`
      )
      .pipe(
        catchError((e) => {
          console.log(
            'Error obteniendo todos los archivos de la oferta académica',
            e.error.mensaje,
            'error'
          );
          return throwError(e);
  ...
  findByStateFile(statefile: string): Observable<any> {
    return this.http
      .get<any>(this.endPoint + this.offers + `?stateFile=${statefile}`)
      .pipe(
        catchError((e) => {
          console.log(
            'Error obteniendo los archivos de la oferta académica por estado ',
            e.error.mensaje,
            'error'
          );
          return throwError(e);
  ...
  updateStateFile(idFile: number, stateFile: string): Observable<any> {
    ...
    return this.http
      ...
      .pipe(
        catchError((e) => {
          console.log(
            'Error actualizando el estado del archivo de la oferta académica:',
            e.error.mensaje,
            'error'
          );
          return throwError(e);
  ...
  getAllPrograms(): Observable<any> {
    return this.http
      .get<any>(this.endPointProgram + '', { responseType: 'json' })
      .pipe(
        catchError((e) => {
          console.log(
            'Error obteniendo todos los programas',
            e.error.mensaje,
            'error'
          );
          return throwError(e);
