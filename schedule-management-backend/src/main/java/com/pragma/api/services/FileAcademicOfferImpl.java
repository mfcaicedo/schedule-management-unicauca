...
public class FileAcademicOfferImpl implements IFileAcademicOffer {
    ...
    @Autowired
    public FileAcademicOfferImpl(ICourseRepository iCourseRepository, IPersonRepository iPersonRepository,
            ISubjectRepository iSubjectRepository, IEnvironmentRepository iEnvironmentRepository,
    ...
    private List<String> processFile(List<FileRowAcademicOffer> logs) {
        ...
        for (FileRowAcademicOffer log : logs) {
             ...
             */
            // if (iPersonRepository.existsById(log.getPersonCode()) &&
            // iSubjectRepository.existsById(log.getSubjectCode())) {
            ...
            // SI ALGUNO DE LOS CODIGOS DE LOS PROFESORES O EL CODIGO DE LA MATERIA NO
            // EXISTE NO SE CONTINUA CON EL PROCESO
            if (!errorPersonCode && !errorSubjectCode) { // Si no hay error se continua
                ...
                Course course = new Course(log.getGroup(), log.getCapacity(), log.getWeeklyOverload(),
                        log.getDescription());
